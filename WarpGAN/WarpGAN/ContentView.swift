...
func testModel(buffer:CVImageBuffer)->warpganOutput?{
    do{
        let config = MLModelConfiguration()
        let model = try warpgan(configuration: config)
        let prediction = try model.prediction(images_B: buffer)
        return prediction
    } catch {
        print(error)
    }
        ...
        .onChange(of: img) { newValue in
            ...
            Task{
                ...
                if let draw = testModel(buffer: buffer) {
                    resultImg = UIImage(pixelBuffer:draw.image)
                }
                isProcessing = false
            }
